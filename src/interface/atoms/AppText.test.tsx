...
import { type NativeSyntheticEvent, type NativeTouchEvent } from "react-native";
...
describe("<AppText />", () => {
  ...
  it("applies maxFontSizeMultiplier prop", () => {
    const { getByText } = render(
      <AppText maxFontSizeMultiplier={2}>Test Text</AppText>,
    );
  ...
  it("handles onPress event", () => {
    ...
    const { getByText } = render(
      <AppText onPress={mockOnPress}>Test Text</AppText>,
    );
  ...
  it("applies style prop", () => {
    const { getByText } = render(
      <AppText style={{ color: "red" }}>Test Text</AppText>,
    );
  ...
  it("applies numberOfLines prop", () => {
    const { getByText } = render(
      <AppText numberOfLines={1}>Test Text</AppText>,
    );
